...
resource "aws_vpc" "eks_vpc" {
  ...
  tags = {
    Name                              = "eks-vpc"
    "kubernetes.io/cluster/tier4-cluster" = "owned"
  }
}
...
# EKS Cluster
resource "aws_eks_cluster" "tier4" {
  name     = "tier4-cluster"
...
# Node Group
resource "aws_eks_node_group" "tier4_nodes" {
  cluster_name    = aws_eks_cluster.tier4.name
  node_group_name = "tier4-node-group"
